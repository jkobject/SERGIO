    ...
    def build_rnd(self, range_dict={"L": [1, 2.5], "H": [3.5, 5]}):
        ...
        for mr in self.mrNames_:
            for ct in range(self.nTypes_):
                ...
                self.profile[mr][ct] = np.random.uniform(
                    low=currRange[0], high=currRange[1], size=1
                )[0]

        self.init_ = True

    def build_rnd_complex(
        self, nMarker_per_type, nLow, nHigh, low=[1, 2.5], high=[3.5, 5]
    ):
        if self.init_:
            print("profile is already initilized.")
            return
        ...
        if total == 0:
            raise ValueError(
                "Specified parameters do not require complex randomization, use 'build_rnd' instead."
            )
        ...
        conds = (
            ...
            + ["h"] * nHigh
            + ["l"] * nLow
            + ["mrk_{}".format(i) for i in range(self.nTypes_)] * nMarker_per_type
        )
        ...
        for mr, cond in zip(self.mrNames_, conds):
            if cond == "mu":
                ...
                for ct in range(self.nTypes_):
                    if ct in hInds:
                        self.profile[mr][ct] = np.random.uniform(
                            low=high[0], high=high[1], size=1
                        )[0]
                    else:
                        self.profile[mr][ct] = np.random.uniform(
                            low=low[0], high=low[1], size=1
                        )[0]
