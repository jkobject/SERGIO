...
class scNoise(object):
    ...
    def _loss(
        ...
        synth_expr,
        real_expr,
        stats=["library", "zeroG", "zeroC", "meanG", "varG"],
    ):
        loss = 0
        weights = [1, 1, 1, 1, 1]
    ...
    def _eval_match(self, params, sample):
        curr_synth = (
            self.sample(self.synth_expr_, sample) if sample else self.synth_expr_
        )
        curr_real = self.sample(self.real_expr_, sample) if sample else self.real_expr_
        currNoisy = self._add_noise(
            expr=curr_synth,
            prob_o=params[0],
            mean_o=params[1],
            scale_o=params[2],
    ...
    def match(self, real, synth_clean, tol_param=0.01, tol_iter=200, sample=None):
        ...
        self.noisy_expr = self._add_noise(
            expr=self.synth_expr_,
    ...
    def plot(self):
        stats = {}
        for expr, key in zip(
            [self.synth_expr_, self.real_expr_, self.noisy_expr],
            ["clean", "real", "noisy"],
        ):
