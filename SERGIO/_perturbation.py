...
class Perturbation(sergio):
    ...
    def __init__(self, relative_path="SERGIO") -> None:
        ...
        self.data_path = os.path.join(
            relative_path, "data"
        )  # path where simulation are saved/loaded
    ...
    def initialise_parameters(self, grn, basal_prod_crispri=0.2, basal_prod_crispra=7):
        ...
        self.nodes_2perturb = np.array(n)[
            np.array(d) > 0
        ]  # it is the array of nodes which regulates at least a node
        logger.info(
            "Number of nodes will be perturbed is " + str(len(self.nodes_2perturb))
        )
    ...
    @staticmethod
    def _single_perturbation(
        grn, mr_profs, target_gene, basal_prod, nCells, cutting=True
    ):
        """
        ...
        if cutting:
            ...
            links_2_remove = list(
                filter(
                    lambda x: x.split("-")[1] == target_gene,
                    new_grn.attr_["interactions"].keys(),
                )
            )
            [
                new_grn.attr_["interactions"].pop(link) for link in links_2_remove
            ]  # remove interaction from GRN
        ...
        new_grn.init(
            crispr, update_half_resp=False
        )  # we set update_half_resp = False because the half_response has already been set in the wt.
    ...
    def perturbation_all(self, nCells, multiprocess=True):
        ...
        grn = copy.deepcopy(
            self.grn_
        )  # it is the grn that I pass to perturbation experiments
        nGenes = len(grn.attr_["genes"])
        self.nCells_ = [
            nCells
        ]  # it is only one cell type, for more, do [nCells]*#celltypes
        ...
        logger.info(
            "Time of execution for wt is  %s seconds" % (time.time() - start_time)
        )

        def wrapper_pert_funct(target_gene, basal_prod):
            return (
                self._single_perturbation(
                    grn=grn,
                    mr_profs=mr_profs,
        ...
        if multiprocess:
            with Pool() as pool:
                self.crispri_data = np.array(
                    list(
                        pool.map(
                            partial(
                                wrapper_pert_funct, basal_prod=self.basal_prod_crispri
                            ),
                            self.nodes_2perturb,
                        )
                    )
                )
                self.crispra_data = np.array(
                    list(
                        pool.map(
                            partial(
                                wrapper_pert_funct, basal_prod=self.basal_prod_crispra
                            ),
                            self.nodes_2perturb,
                        )
                    )
            ...
            self.crispri_data = np.zeros(
                shape=(len(self.nodes_2perturb), nGenes, nCells)
            )
            self.crispra_data = np.zeros(
                shape=(len(self.nodes_2perturb), nGenes, nCells)
            )
            for i, target_gene in tqdm(enumerate(self.nodes_2perturb)):
                ...
                self.crispri_data[i] = (
                    self._single_perturbation(
                        ...
                        mr_profs=mr_profs,
                        target_gene=target_gene,
                        basal_prod=self.basal_prod_crispri,
                        nCells=nCells,
                        cutting=True,
                    )
                    .getSimExpr()
                ...
                self.crispra_data[i] = (
                    self._single_perturbation(
                        ...
                        mr_profs=mr_profs,
                        target_gene=target_gene,
                        basal_prod=self.basal_prod_crispra,
                        nCells=nCells,
                        cutting=True,
                    )
                    .getSimExpr()
    ...
    def draw_net(G, offset=0.05, node_size=1000, **kwargs):
        ...
        nx.draw_networkx_edges(
            G,
            pos=nodePos,
            edgelist=np.array(non_bi_edges)[non_bi_weight > 0],
            edge_color="green",
            arrowsize=20,
            node_size=node_size,
        )
        ...
        nx.draw_networkx_edges(
            G,
            pos=new_nodePos,
            edgelist=np.array(bi_edges)[bi_weight > 0],
            edge_color="green",
            arrowsize=20,
        )
        new_nodePos = {}
    ...
    def load(self, nGenes, nCells):
        ...
        self.__dict__.update(
            dataPickle
        )  # add loaded dictionary to methods of the class
        return self
