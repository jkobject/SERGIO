...
class sergio:
    def __init__(
        ...
        self.dyn_ = dynamics
        self.nConvSteps = np.zeros(
            number_bins
        )  # This holds the number of simulated steps till convergence
        if dynamics:
            self.bifurcationMat_ = np.array(bifurcation_matrix)
            self.binOrders_ = []
            self.binDict = {}
            for b in range(self.nBins_):
                self.binDict[b] = np.zeros(
                    self.nGenes_,
                ).tolist()
        ############
            ...
            if shared_coop_state <= 0:
                for row in reader:
                    ...
                    for regId, K, C_state in zip(
                        ...
                        currInteraction.append(
                            (int(float(regId)), float(K), float(C_state), 0)
                        )  # last zero shows half-response, it is modified in another method
                    ...
                    self.graph_[int(float(row[0]))][
                        "level"
                    ] = -1  # will be modified later
                ...
                for indRow, row in enumerate(reader):
                    ...
                    for (
                        regId,
                        K,
                    ) in zip(row[2 : 2 + nRegs], row[2 + nRegs : 2 + 2 * nRegs]):
                        currInteraction.append(
        ...
        if reg_conc == 0:
            ...
            if repressive:
                return 1 - np.true_divide(
                    np.power(reg_conc, coop_state),
                    (
                        np.power(half_response, coop_state)
                        + np.power(reg_conc, coop_state)
                    ),
                )
            else:
        ...
        for g in currGenes:
            if g[0].Type == "MR":
                ...
                for bIdx, rate in enumerate(allBinRates):
                    g[bIdx].append_Conc(
                        np.true_divide(rate, self.decayVector_[g[0].ID])
                    )
                ...
                for bIdx in range(self.nBins_):
                    ...
                    for interTuple in params:
                        ...
                        rate += abs(interTuple[1]) * self.hill_(
                            meanExp, interTuple[3], interTuple[2], interTuple[1] < 0
                        )
        ...
        if type == "MR":
            ...
            for tupleIdx, rIdx in enumerate(regIndices):
                ...
                for colIdx, bIdx in enumerate(binIndices):
                    hillMatrix[tupleIdx, colIdx] = self.hill_(
                        regGene_allBins[bIdx].Conc[currStep],
                        params[tupleIdx][3],
                        params[tupleIdx][2],
                        params[tupleIdx][1] < 0,
                    )
            ...
            for gi, g in enumerate(sim_set):
                ...
                if self.noiseType_ == "sp":
                    ...
                    amplitude = np.multiply(
                        self.noiseParamsVector_[gID], np.power(prod_rate, 0.5)
                    )
                    ...
                    amplitude_p = np.multiply(
                        self.noiseParamsVector_[gID], np.power(prod_rate, 0.5)
                    )
                    amplitude_d = np.multiply(
                        self.noiseParamsVector_[gID], np.power(decay, 0.5)
                    )

                    noise = np.multiply(amplitude_p, dw_p) + np.multiply(
                        amplitude_d, dw_d
                    )

                curr_dx = (
                    self.dt_ * (prod_rate - decay) + np.power(self.dt_, 0.5) * noise
                )
    ...
    def find_bin_order_(self, bifurcation_matrix):
        ...
        try:
            ...
            print(
                "ERROR: Bifurication graph is assumed to be acyclic, but a cyclic graph was passed."
            )
            sys.exit()
                ...
                for g in currGenes:
                    if g[0].Type == "MR":
                        ...
                        self.binDict[binID][g[0].ID].set_ss_conc_U(
                            np.true_divide(currRate, self.decayVector_[g[0].ID])
                        )
                        self.binDict[binID][g[0].ID].set_ss_conc_S(
                            self.ratioSp_[g[0].ID]
                            * np.true_divide(currRate, self.decayVector_[g[0].ID])
                        )
                    else:
                        params = self.graph_[g[0].ID]["params"]
                        ...
                        for interTuple in params:
                            ...
                            currRate += abs(interTuple[1]) * self.hill_(
                                meanExp, interTuple[3], interTuple[2], interTuple[1] < 0
                            )
    ...
    def populate_with_parentCells_(self, binID):
        ...
        if np.count_nonzero(parentBins) > 1:
            ...
            nPopulation = int(
                round(self.bifurcationMat_[parentBinID, binID] * self.nSC_)
            )
    ...
    def calculate_prod_rate_U_(self, gID, binID, num_c_to_evolve):
        ...
        if type == "MR":
            ...
            for tupleIdx, ri in enumerate(regIndices):
                currRegConc = [
                    self.binDict[binID][ri].Conc[i][-1] for i in range(num_c_to_evolve)
                ]
                for ci, cConc in enumerate(currRegConc):
                    hillMatrix[tupleIdx, ci] = self.hill_(
                        cConc,
                        params[tupleIdx][3],
                        params[tupleIdx][2],
                        params[tupleIdx][1] < 0,
    ...
    def resume_after_convergence(self, binID):
        if (
            self.binDict[binID][0].simulatedSteps_
            < self.sampling_state_ * self.nConvSteps[binID]
        ):
            return True
        else:
            return False
    ...
    def dynamics_CLE_simulator_(self, binID):
        ...
        converged = False
        sim_set = self.binDict[
            binID
        ]  # this is a list of gene object that we are simulating
        nc = len(
            sim_set[0].Conc
        ...
        while resume:
            for gID, g in enumerate(sim_set):
                ...
                decay_S = (
                    np.true_divide(self.decayVector_[gID], self.ratioSp_[gID]) * currS
                )
                ...
                if self.noiseType_ == "sp":
                    ...
                    amplitude = np.multiply(
                        self.noiseParamsVector_[gID], np.power(prod_rate_U, 0.5)
                    )
                    ...
                    amplitude_d = np.multiply(
                        self.noiseParamsVector_[gID], np.power(decay_U, 0.5)
                    )
                    noise_U = np.multiply(amplitude_p, dw_p) + np.multiply(
                        amplitude_d, dw_d
                    )
                ...
                if self.noiseTypeSp_ == "sp":
                    ...
                    dw = np.random.normal(size=nc)
                    amplitude = np.multiply(
                        self.noiseParamsVectorSp_[gID], np.power(prod_rate_S, 0.5)
                    )
                    ...
                    amplitude_d = np.multiply(
                        self.noiseParamsVectorSp_[gID], np.power(decay_S, 0.5)
                    )
                    noise_S = np.multiply(amplitude_p, dw_p) + np.multiply(
                        amplitude_d, dw_d
                    )

                curr_dU = (
                    self.dt_ * (prod_rate_U - decay_U)
                    + np.power(self.dt_, 0.5) * noise_U
                )
                curr_dS = (
                    self.dt_ * (prod_rate_S - decay_S)
                    + np.power(self.dt_, 0.5) * noise_S
                )
    ...
    def getExpressions_dynamics(self):
        ...
        for bi in range(self.nBins_):
            ...
            randCells = np.random.choice(
                range(nSimSteps), size=self.nSC_, replace=False
            )
    ...
    def lib_size_effect(self, scData, mean, scale):
        ...
        libFactors = np.random.lognormal(
            mean=mean, sigma=scale, size=(self.nBins_, self.nSC_)
        )
    ...
    def dropout_indicator(self, scData, shape=1, percentile=65):
        ...
        prob_ber = np.true_divide(
            1, 1 + np.exp(-1 * shape * (scData_log - log_mid_point))
        )
    ...
    def lib_size_effect_dynamics(self, U_scData, S_scData, mean, scale):
        ...
        for binExprU, binExprS, binFactors in zip(U_scData, S_scData, libFactors):
            ...
            binFactors = np.true_divide(
                binFactors, normalizFactors_U + normalizFactors_S
            )
            binFactors = binFactors.reshape(1, self.nSC_)
            binFactors = np.repeat(binFactors, self.nGenes_, axis=0)
