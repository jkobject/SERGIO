...
class sergio(object):
    ...
    def simulate(
        self, nCells, noise_s, noise_u=None, safety_iter=50, scale_iter=10, dt=0.01
    ):
    ...
    def _iter_ss_gpu(self, noise_ss, dt, cTypes):
        ...
        newX = (
            X
            + (P - D) * dt
            + (cp.multiply(cp.sqrt(P), rndP) + cp.multiply(cp.sqrt(D), rndD))
            * noise_ss
            * cp.sqrt(dt)
        )
    ...
    def _simulate_ss_gpu(
        self, nCells, noise_ss, dt=0.01, safety_iter=50, scale_iter=10
    ):
        """
        # TODO: make sure nCells is already np.array
        """
    ...
    def lib_size_effect(self, scData, mean, scale):
        ...
        if len(libFactors) != scData.shape[1]:
            raise ValueError(
                "Number of cells in scData expected to be "
                + str(len(libFactors))
                + ", having  "
                + str(scData.shape[1])
                + " instead"
        ...
        for cell_idx, cellFactors in enumerate(libFactors):
            ...
            cellFactors = np.repeat(
                cellFactors, len(self.gNames_), axis=0
            )  # create a matrix with repeated rows, where value is given by the technical noise term for each cell
